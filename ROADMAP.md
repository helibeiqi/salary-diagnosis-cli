...
> 版本：v0.2.2（2026-09-04 更新）　｜　维护者：何李贝奇　｜　口径：北京 / 中国 A 股语境（红=偏高/成本风险，绿=偏低/流失风险）
>
...
- [x] **CLI 易用性**：✅ 已落地（v0.2.2）`run_agent.py --demo` 一键跑通——自带合成模拟样例（synthetic 分级，外发零风险）、零交互（固定 预算5%/策略A/海氏/自动映射）、样例缺失时自动按固定种子重新生成（实测与仓库基线逐字节一致）；`--check-data-guard` 预检外发风险仍待做。
