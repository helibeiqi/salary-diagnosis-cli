...
> 版本：v0.2（2026-09-02 更新）　｜　维护者：何李贝奇　｜　口径：北京 / 中国 A 股语境（红=偏高/成本风险，绿=偏低/流失风险）
>
...
### P2 — 体验 / 可扩展性
- [ ] **R6 单测与 CI 覆盖**：补 `tests/test_level_infer.py`（启发式 + 覆盖率）、`tests/test_report_guard.py`（真实/脱敏水印分支）、脱敏往返测试（脱敏→重加载→报告标注 sanitized）；接入 GitHub Actions 跑 `pytest`，在 `src/data/` / `assets/` 误提交时 fail。
